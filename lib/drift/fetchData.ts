...
/* Get the Address for Drift User Account */
export function getUserPda(user: PublicKey, subAccountId: BN): PublicKey {
    const [userPda] = PublicKey.findProgramAddressSync(
        [Buffer.from("user"), user.toBuffer(), subAccountId.toArrayLike(Buffer, 'le', 2)],
        DRIFT_PROGRAM_ID
    );
...
/* Get the Address for Drift User Stats Account */
export function getUserStatsPda(user: PublicKey): PublicKey {
...
/* Get the Drift User Account Data */
export async function getUserData(connection: Connection, userPda: PublicKey) {
 ...
 * Decode & Parse On Chain Data for Drift User Account
 */
...
function decodeUser(buffer: Buffer): UserAccount {
	let offset = 8;
	...
	for (let i = 0; i < 32; i++) {
		...
		if (orderStatusNum === 0) {
            ...
            throw new Error(`Unknown order status: ${orderStatusNum}`);
        }
		offset += 1;
		...
		if (orderTypeNum === 0) {
			...
			throw new Error(`Unknown order type: ${orderTypeNum}`);
		}
		offset += 1;
		...
		if (triggerConditionNum === 0) {
			...
			throw new Error(`Unknown trigger condition: ${triggerConditionNum}`);
		}
		offset += 1;
