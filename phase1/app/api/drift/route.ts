...
export async function GET(req: NextRequest) {
  ...
  const address = searchParams.get("user");

  if(!process.env.PRIVATE_SOLANA_RPC_URL) {
     return new Response(JSON.stringify({ error: "Solana RPC Env Not Provided" }), {
      status: 400,
    });
  }
  ...
  // const privateKeyEnv = process.env.NEXT_PUBLIC_SOLANA_PRIVATE_KEY || "";
